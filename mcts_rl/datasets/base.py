...
def jsonlines_load(fname: str) -> list[dict[str, Any]]:
...
class RawSamplePost(TypedDict, total=False):
    # tensor
    prompt: NotRequired[torch.Tensor | list[torch.Tensor]]  # either `input` or `dialogue` should be provided
    """User input text."""
    input_ids_list: NotRequired[list[torch.Tensor]]
    """Assistant answer text."""
